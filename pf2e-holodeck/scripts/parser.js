/** Localize PF2E-HOLODECK keys for Combat Forensics */
function CF_L(key, data) {
...
class CombatForensicsApp extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.api.ApplicationV2) {
    static DEFAULT_OPTIONS = {
        ...
        window: { title: "PF2E-HOLODECK.Forensics.Title", resizable: true },
        actions: {
            ...
            clearHistory: async function() {
                if (game.user.isGM) {
                    ...
                    this.expandedActors = {};
                    this.render({ force: true });
                    ui.notifications.warn(CF_L("CF.Purged"));
                }
            },
            swapAllegiance: async function(event, target) {
                ...
                if (targetLedger && targetLedger.actors[actorName]) {
                    ...
                    this.render({ force: true });
                    ui.notifications.info(CF_L("CF.Swapped", { name: actorName }));
                }
            },
            createManualLog: async function(event, target) {
                ...
                if (!encName || encName.startsWith("meta")) {
                    return ui.notifications.warn(CF_L("CF.NoMetaLog"));
                }
                ...
                if (!targetLedger) return ui.notifications.error(CF_L("CF.NoLedger"));
                ...
                const confirm = await foundry.applications.api.DialogV2.prompt({
                    window: { title: "PF2E-HOLODECK.Dialog.InjectManual" },
                    content: content,
                    ok: {
                        label: "Inject Entry",
                        icon: "fas fa-syringe",
                ...
                ui.notifications.info(CF_L("CF.ManualInjected"));
                this.render(true);
            },
            deleteLog: async function(event, target) {
                ...
                if (logIndex === -1) return ui.notifications.warn(CF_L("CF.LogMissing"));

                const lEntry = targetLedger.masterLog[logIndex];
                
                const confirm = await foundry.applications.api.DialogV2.confirm({
                    window: { title: "PF2E-HOLODECK.Dialog.DeleteLog" },
                ...
                ui.notifications.info(CF_L("CF.LogDeleted"));
                this.render(true);
            },
            deleteEncounter: async function() {
                ...
                if (encName === "current") {
                    const confirm = await foundry.applications.api.DialogV2.confirm({
                        window: { title: "PF2E-HOLODECK.Dialog.ArchiveWipe" },
                        content: `<p>Force an archive and wipe the current live combat tracker?</p>`,
                        rejectClose: false
                    });
                    if (!confirm) return;
                    await window.CombatParser.saveArchive();
                    ui.notifications.info(CF_L("CF.ArchivedWiped"));
                    this.render(true);
                    return;
                }

                if (encName === "exploration" || encName.startsWith("meta")) {
                    return ui.notifications.warn(CF_L("CF.CantDeleteMeta"));
                }

                const confirm = await foundry.applications.api.DialogV2.confirm({
                    window: { title: "PF2E-HOLODECK.Dialog.DeleteEncounter" },
                ...
                this.selectedEncounter = "exploration";
                ui.notifications.info(CF_L("CF.EncounterDeleted"));
                this.render(true);
            },
            reassignLog: async function(event, target) {
                ...
                if (selectedIds.length === 0) return ui.notifications.warn(CF_L("CF.NoLogsSelected"));
                ...
                if (!logEntry) return ui.notifications.warn(CF_L("CF.LogMissing"));
                ...
                const dialog = new foundry.applications.api.DialogV2({
                    window: { title: "PF2E-HOLODECK.Dialog.MassAudit" },
                    position: { width: 500 },
            ...
            exportData: async function() {
                if (!game.user.isGM) return ui.notifications.warn(CF_L("CF.OnlyGMJournal"));
                const hDb = game.settings.get('pf2e-holodeck', 'combatHistory') || {};
                const sDb = game.settings.get('pf2e-holodeck', 'holodeckHistory') || {};
                const eDb = game.settings.get('pf2e-holodeck', 'explorationHistory') || {};
                
                if (Object.keys(hDb).length === 0 && Object.keys(sDb).length === 0 && Object.keys(eDb).length === 0) return ui.notifications.warn(CF_L("CF.NoDataArchive"));
                ...
                if (journal) { journal.sheet.render(true); ui.notifications.info(CF_L("CF.JournalOk")); }
            },
            downloadJSON: async function() {
                if (!game.user.isGM) return ui.notifications.warn(CF_L("CF.OnlyGMJSON"));
                const hDb = game.settings.get('pf2e-holodeck', 'combatHistory') || {};
                const sDb = game.settings.get('pf2e-holodeck', 'holodeckHistory') || {};
                const eDb = game.settings.get('pf2e-holodeck', 'explorationHistory') || {};
                
                if (Object.keys(hDb).length === 0 && Object.keys(sDb).length === 0 && Object.keys(eDb).length === 0) return ui.notifications.warn(CF_L("CF.NoDataExport"));
                ...
                ui.notifications.info(CF_L("CF.JSONStarted"));
            },
            importData: async function() {
                if (!game.user.isGM) return ui.notifications.warn(CF_L("CF.OnlyGMImport"));
                ...
                const processImport = async (data) => {
                    if (!data || (!data.combatHistory && !data.combat && !data.explorationHistory && !data.holodeckHistory)) {
                        return ui.notifications.error(CF_L("CF.BadFormat"));
                    }
                    ...
                    ui.notifications.info(CF_L("CF.ImportOk"));
                    this.render({ force: true });
                };

                const dialog = new foundry.applications.api.DialogV2({
                    window: { title: "PF2E-HOLODECK.Dialog.ImportArchives" },
                    position: { width: 450 },
                    content: content,
                    ...
                    inputJson.addEventListener('change', (e) => {
                        ...
                        reader.onload = async (ev) => {
                            try {
                                ...
                                ui.notifications.error(CF_L("CF.ParseFail"));
                                console.error(err);
                            }
                        };
...
Hooks.once('init', () => {
    ...
    game.keybindings.register('pf2e-holodeck', 'toggle-analytics', {
        name: "PF2E-HOLODECK.Key.ToggleForensics", hint: "PF2E-HOLODECK.Key.ToggleForensicsHint",
...
Hooks.on('renderCombatTracker', async (app, html, data) => {
    ...
    btn.innerHTML = `<i class="fas fa-microscope"></i> ${CF_L("Forensics.BtnLabel")}`;
